...
class AMPVAEPPO:
    ...
    def __init__(
        ...
        params = [
            ...
            {"params": self.vae.parameters(), "lr": learning_rate, "name": "vae"},
        ]
    ...
    def update(self):  # noqa: C901
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(generator, amp_policy_generator, amp_expert_generator):
            ...
            p_boot_mean = self.p_boot.mean().clone().detach()
            obs_full_batch = torch.cat(
                (
                    p_boot_mean * vae_code_vel + (1 - p_boot_mean) * critic_obs_batch[:, 0:3],
                    p_boot_mean * vae_code_mass + (1 - p_boot_mean) * critic_obs_batch[:, -1:],
                    vae_code_latent,
                    actor_obs_batch,
                ),
                dim=-1,
            ).detach()
            self.actor_critic.act(obs_full_batch, masks=masks_batch, hidden_states=hid_states_batch[0])
